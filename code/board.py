...
class Game_Logic:
    ...
    def place_player_stone(self, mouse_x, mouse_y):
        global game, bots,rxhint,ryhint
        if self.MODE == 0 or self.CURRENT_TURN == self.WHO_IS_CPU:
            return
        ...
        if self.stone_matrix[j][i] == 0:
            ...
            if not bot_move:
                ...
                rxhint = bot_move.point.col - 1
                ryhint = game.board.num_rows - bot_move.point.row
            old=count(game,self.CURRENT_TURN)
            game = game.apply_move(Player_move)
            new=count(game,self.CURRENT_TURN)
            player_prisonesr[player_turn[self.CURRENT_TURN]]+=old-new
            return True
    ...
    def update_stone_matrix(self, mouse_x, mouse_y):
        ...
        if self.MODE == 1 and self.CURRENT_TURN == self.WHO_IS_CPU and not self.first_stone:
            self.cpu_hint_x_y = [self.get_x_corresponding_to_col(rxhint), self.get_y_corresponding_to_row(ryhint)]
            self.hint_text = "    The best \n" \
                             "\n   position to \n" \
                             "\n     play was\n" \
                             "\n       (" + str(rxhint + 1) + \
                             "," + str(BOARD_SIZE[1] - ryhint) + ")"
        else:
            # remove hint
    ...
    def keypressed(self, key):
        global Resign
        global game
        if self.MODE == 1 and self.CURRENT_TURN != self.WHO_IS_CPU:
            if key == arcade.key.P:
                self.change_turn()
                self.pas = 1
                mymove = Move(is_pass=True)
    ...
    def draw_info(self):
        if self.MODE == 1:
            arcade.draw_text("Press R to resign",SCREEN_WIDTH-250,PLAYABLE_SCREEN_HEIGHT/2 - 2*FONT_SIZE, TEXT_COLOR,
                             1.5*FONT_SIZE)
            arcade.draw_text("Press P to pass", SCREEN_WIDTH-250, PLAYABLE_SCREEN_HEIGHT/2 + 2*FONT_SIZE, TEXT_COLOR,
                             1.5*FONT_SIZE)
            arcade.draw_text("Press Esc to exit",SCREEN_WIDTH-250,PLAYABLE_SCREEN_HEIGHT/2, TEXT_COLOR, 1.5*FONT_SIZE)
    ...
    def draw_hint(self):
        ...
        # for the hint text
        if not self.first_stone and not (self.WHO_IS_CPU == 2 and self.second_stone):
            start_x = 0
